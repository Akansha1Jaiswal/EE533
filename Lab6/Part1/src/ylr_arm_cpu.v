...
module ylr_arm_cpu 
   ...
   wire [31:0] dmem_data_ext;

   wire        dmem_re_ext;
   wire [31:0] dmem_rdata_ext;
   
   wire [3:0]  regfile_addr_ext;
   wire        regfile_re_ext;
   wire [31:0] regfile_rdata_ext;
   ...
   #( 
      ...
      .TAG                 (`YLR_ARM_CPU_BLOCK_ADDR),          // Tag -- eg. MODULE_TAG
      .REG_ADDR_WIDTH      (`YLR_ARM_CPU_REG_ADDR_WIDTH),     // Width of block addresses -- eg. MODULE_REG_ADDR_WIDTH
      ...
      // --- SW regs interface
      .software_regs    ({dmem, imem_data, address, command}), //start from the last and move to the top according to xml declarations

      // --- HW regs interface
      .hardware_regs    ({dmem_read, regfile_read}), //start from the last same as above
...
/* 	command [0] - Write IMem
    ...
    command [2] - Read DMem
	command [3] - Read RegFile*/
    
	wire write_imem    = command [0];
	wire write_dmem    = command [1];
	wire read_dmem     = command [2];
	wire read_regfile  = command [3];
	

    // Connecting BRAMs to Software/Hardware regs - Signals from the command reg
	...
	always @(posedge clk) begin
	    if (reset) begin
           ...
           dmem_read <= dmem_rdata_ext;
        end
		else if (regfile_re_ext) begin
		   regfile_read <= regfile_rdata_ext;
		end
    end
