...
assign stall        = hazard_stall | tensor_stall;
...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (EX1_tensor_enable && !tensor_pending) begin
            tensor_pending       <= 1'b1;
...
/* data_Rmemory DM (
    .clk        (clk),
    .mem_read   (EX2_MEM_mem_read),
    .mem_write  (EX2_MEM_mem_write_dly),
    .addr       (EX2_MEM_ALU_dly[31:0]),
    .write_data (EX2_MEM_store_data_dly),
    .read_data  (mem_data)
);  */
...
data_Rmemory DM (
    ...
    // PORT B -- write
    .clkb   (clk),
    .addrb  (EX2_MEM_ALU_dly[7:0]),
    .dinb   (EX2_MEM_store_data_dly),
    .doutb  (),
    .web    (EX2_MEM_mem_write_dly)
);
